...
# Get config
curl --connect-timeout 10 -s --compressed "https://assets.twitch.tv/eppo/api/flag-config/v1/config?sdkName=js-sdk-client&sdkVersion=3.1.2&apiKey=$api_key" | jq -M --tab 'del(.createdAt) | .flags |= (to_entries | sort_by(.key) | from_entries)' > config.json
if [ -s config.json ]; then
	git add config.json && git commit -m 'Update config.json'
fi

# Get global badges
curl --connect-timeout 10 -s --compressed --request POST --header 'Client-Id: kimne78kx3ncx6brgo4mv6wki5h1ko' --header 'Content-Type: application/json' --data '{"query":"query GetGlobalBadges { badges { id setID version title description imageURL onClickAction clickURL }}","operationName":"GetGlobalBadges","variables":{}}' 'https://gql.twitch.tv/gql' | jq -M --tab '.data.badges|sort_by(.title, .setID, .version)' > global_badges.json
